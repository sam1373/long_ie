...
def read_apf_file(path: str,
    ...
    for relation in doc.find_all('relation'):
        ...
        for relation_mention in relation.find_all('relation_mention'):
            ...
            if arg1 and arg2:
                relation_list.append(Relation(relation_id, relation_type,
...
def convert(sgm_file: str,
            ...
            time_and_val: bool = False,
            language: str = 'english',
            sentence_level: bool = False) -> Document:
    """Converts a document.
    ...
    #text_start = sentences[0][1]
    #text_end = sentences[-1][2]

    #sentences = [(full_text, text_start, text_end)]
    ...
    # Tokenization
    sentence_tokens = [tokenize(s, entities, events, language=language) for s, ent, evt
    ...
    if not sentence_level:
        ...
        for sen_id, sen in enumerate(sentence_entities):
            for entity in sen:
                if entity.char_offsets_to_token_offsets(doc_tokens):
                    entities_proc.append(entity)
                    mentions_kept.add(entity.mention_id)
        ...
        for event in events:
            ...
            for arg in event.arguments:
                if arg.mention_id in mentions_kept:
                    cleaned_args.append(arg)
            event.arguments = cleaned_args
            if event.trigger.char_offsets_to_token_offsets(doc_tokens):
                events_proc.append(event)

        rel_proc = []

        for relation in relations:
            if relation.arg1.mention_id in mentions_kept and relation.arg2.mention_id in mentions_kept:
                rel_proc.append(relation)

        doc_obj = Sentence(start=doc_tokens[0][0],
                           end=doc_tokens[-1][1],
                           text="\n".join([sent[0] for sent in sentences]),
                           sent_id=doc_id,
                           tokens=[t for _, _, t in doc_tokens],
                           entities=entities_proc,
                           events=events_proc,
                           relations=rel_proc)

        return Document(doc_id, [doc_obj])
    ...
    if sentence_level:
        ...
        for i, (toks, ents, evts, rels, sent) in enumerate(zip(
            ...
            sentence_objs.append(Sentence(start=sent[1],
                                          ...
                                          relations=rels,
                                          events=evts))
        print(doc_id, len(sentence_objs))
    ...
    with open(output_path, 'w', encoding='utf-8') as w:
        for sgm_file in sgm_files:
            ...
            doc = convert(sgm_file, apf_file, time_and_val=time_and_val,
                          language=language, sentence_level=sentence_level)
        ...
        for line in r:
            ...
            for sentence in doc['sentences']:
                ...
                for event in sentence['events']:
                    for arg in event['arguments']:
                        if arg['mention_id'] in entity_text:
                            arg['text'] = entity_text[arg['mention_id']]
                        else:
                            arg['text'] = None
                            print("argument not found:", arg['mention_id'])
                    ...
                    relations.append({
                        ...
                        'arguments': [
                            ...
                            },
                            {
                                ...
                                #'mention_id': relation['arg2']['mention_id'],
                                'text': relation['arg2']['text'],
                                'role_id': relation['arg2']['role']
                            }
                        ]
                    })
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sentence_level', action='store_true',
                        help='Switch back to sentence level for debugging')
    ...
    convert_batch(input_dir, json_path, time_and_val=args.time_and_val,
                  language=args.lang, sentence_level=args.sentence_level)
