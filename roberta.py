...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config, fast=False):
        ...
        self.fast_attn_fn = FastAttention(dim_heads=self.attention_head_size,
                                          nb_features=256,
                                          causal=False)
    ...
    def forward(
        ...
        if self.fast == False:
            ...
            context_layer = self.fast_attn_fn(query_layer, key_layer, value_layer).permute(0, 2, 1, 3).contiguous()
            new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
            context_layer = context_layer.view(*new_context_layer_shape)
...
class RobertaAttention(nn.Module):
    def __init__(self, config, fast=False):
        super().__init__()
        self.self = RobertaSelfAttention(config, fast)
...
class RobertaLayer(nn.Module):
    def __init__(self, config, fast=False):
        ...
        self.seq_len_dim = 1
        self.attention = RobertaAttention(config, fast)
...
class RobertaEncoder(nn.Module):
    def __init__(self, config, fast):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaLayer(config, fast) for _ in range(config.num_hidden_layers)])
...
class RobertaModelPlus(RobertaPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True, fast=True):
        super().__init__(config)
        self.config = config

        self.embeddings = RobertaEmbeddings(config)
        self.encoder = RobertaEncoder(config, fast)
